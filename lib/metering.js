...
export async function commitTranscribeUsage(env = {}, session = {}, input = {}, result = {}, guard = {}) {
  ...
  await addUsage(env, monthlyUsageKey(user.userId), {
    ...
    estimatedCostUsd: estimateCost(input.duration || result.durationSeconds, provider) || 0,
...
function getPlanLimits(env = {}, plan = defaultPlan) {
  ...
  return {
    monthlyTranscribeMinutes: readLimit(env, `PODNOTE_${normalizedPlan}_MONTHLY_TRANSCRIBE_MINUTES`, readLimit(env, "PODNOTE_MONTHLY_TRANSCRIBE_MINUTES", 60)),
    dailyTranscribeMinutes: readLimit(env, `PODNOTE_${normalizedPlan}_DAILY_TRANSCRIBE_MINUTES`, readLimit(env, "PODNOTE_DAILY_TRANSCRIBE_MINUTES", 120)),
    maxSingleTranscribeMinutes: readLimit(env, `PODNOTE_${normalizedPlan}_MAX_SINGLE_TRANSCRIBE_MINUTES`, readLimit(env, "PODNOTE_MAX_SINGLE_TRANSCRIBE_MINUTES", 120)),
    monthlyAnalyzeCount: readLimit(env, `PODNOTE_${normalizedPlan}_MONTHLY_ANALYZE_COUNT`, readLimit(env, "PODNOTE_MONTHLY_ANALYZE_COUNT", 30)),
