...
async function inspectAudio(audioUrl, signal) {
  try {
    const response = await fetch(audioUrl, {
      method: "HEAD",
      headers: { "User-Agent": "PodNote/0.15.0" },
      signal
    });
...
function extractDeepgramTranscript(data) {
  ...
  const wordLines = Array.isArray(alternative?.words) && alternative.words.length ? wordsToTimestampedLines(alternative.words) : "";
  const sentences = alternative?.paragraphs?.paragraphs?.flatMap((paragraph) => paragraph.sentences || []) || [];
  if (shouldUseDeepgramSentences(sentences)) {
    return sentencesToTimestampedLines(sentences);
  }

  if (wordLines) return wordLines;
  if (sentences.length) return sentencesToTimestampedLines(sentences, { forceSplit: true });

  const utterances = data?.results?.utterances;
  if (Array.isArray(utterances) && utterances.length) {
    return mergeDeepgramUtterances(utterances);
  }

  return normalizeTranscriptSentence(alternative?.transcript);
}

function shouldUseDeepgramSentences(sentences) {
  if (!sentences.length) return false;
  if (sentences.length < 3) return false;
  const textLengths = sentences.map((sentence) => normalizeTranscriptSentence(sentence.text).length).filter(Boolean);
  if (!textLengths.length) return false;
  const longest = Math.max(...textLengths);
  const average = textLengths.reduce((total, length) => total + length, 0) / textLengths.length;
  return longest <= 140 && average <= 90;
}

function sentencesToTimestampedLines(sentences, options = {}) {
  return sentences
    .flatMap((sentence) => {
      const text = normalizeTranscriptSentence(sentence.text, { terminalPunctuation: true });
      if (!text) return [];
      const chunks = options.forceSplit || text.length > 90 ? splitLongChineseText(text) : [text];
      const start = Number(sentence.start || 0);
      const end = Number(sentence.end || start + Math.max(1, chunks.length * 4));
      const span = Math.max(1, end - start);

      return chunks.map((chunk, index) => {
        const offset = chunks.length > 1 ? (span * index) / chunks.length : 0;
        return `[${secondsToTimestamp(start + offset)}] ${ensureTerminalPunctuation(chunk)}`;
      });
    })
    .filter(Boolean)
    .join("\n");
...
function splitLongChineseText(text) {
  const normalized = normalizeTranscriptSentence(text);
  const chunks = [];
  let current = "";
  ...
  for (const token of normalized.split(/(?<=[。！？!?；;，,、])|(?=\s)/)) {
    ...
    if (!part) continue;
    if (current && `${current}${part}`.length > 44) {
      chunks.push(current);
      current = part;
    } else {
      current = `${current}${needsJoinSpace(current, part) ? " " : ""}${part}`;
    }
  }
  ...
  const fallback = [];
  for (let index = 0; index < normalized.length; index += 38) {
    fallback.push(normalized.slice(index, index + 38));
  }
  return fallback.filter(Boolean);
}

function mergeDeepgramUtterances(utterances) {
  const lines = [];
  let current = null;

  for (const utterance of utterances) {
    const text = normalizeTranscriptSentence(utterance.transcript);
    if (!text) continue;

    const speaker = Number.isFinite(utterance.speaker) ? utterance.speaker : null;
    const shouldStartNew =
      !current ||
      current.text.length >= 46 ||
      /[。.!?！？]$/.test(current.text) ||
      (speaker !== null && current.speaker !== null && speaker !== current.speaker);

    if (shouldStartNew) {
      if (current) lines.push(formatDeepgramLine(current));
      current = { start: utterance.start || 0, speaker, text };
      continue;
    }

    current.text = normalizeTranscriptSentence(`${current.text}${needsJoinSpace(current.text, text) ? " " : ""}${text}`);
  }
...
function formatDeepgramLine(line) {
  const speaker = line.speaker !== null ? `说话人${line.speaker + 1}：` : "";
  return `[${secondsToTimestamp(line.start)}] ${speaker}${ensureTerminalPunctuation(line.text)}`;
}
...
function wordsToTimestampedLines(words) {
  ...
  for (const word of words) {
    ...
    if (isSentenceEnd || current.join("").length >= 42) {
      lines.push(`[${secondsToTimestamp(start)}] ${ensureTerminalPunctuation(normalizeTranscriptSentence(current.join(" ")))}`);
      current = [];
    }
  }

  if (current.length) lines.push(`[${secondsToTimestamp(start)}] ${ensureTerminalPunctuation(normalizeTranscriptSentence(current.join(" ")))}`);
  return lines.join("\n");
}

function normalizeTranscriptSentence(value, options = {}) {
  ...
  if (options.terminalPunctuation) text = ensureTerminalPunctuation(text);
  return text;
}

function ensureTerminalPunctuation(text) {
  if (!text || /[。.!?！？；;：:]$/.test(text)) return text;
  return /[\u3400-\u9fff]/.test(text) ? `${text}。` : text;
}
