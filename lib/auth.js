...
const defaultSessionSecret = "podnote-local-session-secret";
const sessionSeconds = 7 * 24 * 60 * 60;

export function getSessionSecret(env = {}) {
  return String(env.PODNOTE_SESSION_SECRET || defaultSessionSecret);
}

export async function isAuthorized(cookieHeader, secret) {
  return Boolean(await getAuthorizedSession(cookieHeader, secret));
}

export async function getAuthorizedSession(cookieHeader, secret) {
  const token = readCookie(cookieHeader, authCookieName);
  if (!token) return null;
  return verifySessionToken(token, secret);
}

export async function createAuthCookie(session, secret, requestUrl) {
...
export function renderLoginPage(options = {}) {
  const setup = options.setup
    ? `<p class="notice">请先在 Cloudflare 环境变量里设置 <code>PODNOTE_INVITE_CODES</code> 和 <code>PODNOTE_SESSION_SECRET</code>，并建议绑定 <code>PODNOTE_KV</code>。</p>`
    : "";
  const error = options.error ? `<p class="error">邀请码无效，请再试一次。</p>` : "";
...
<html lang="zh-CN">
  ...
  <body>
    <main>
      ...
      <h1>输入邀请码</h1>
      <p>这个版本还在内测中，每个邀请码会绑定独立的转写和分析额度。</p>
      ${setup}
      ${error}
      <form method="post" action="/api/auth/login">
        <input name="inviteCode" type="text" autocomplete="one-time-code" placeholder="邀请码" autofocus />
...
export function unauthorizedJson() {
  return JSON.stringify({ error: "需要邀请码登录" });
}
  ...
  const payload = base64Url(
    new TextEncoder().encode(
      JSON.stringify({
        ...
        inviteHash: session.inviteHash || "",
        plan: session.plan || "free",
        label: session.label || "内测用户",
        expiresAt
      })
    )
  );
...
async function verifySessionToken(token, secret) {
  ...
  const expiresAtNumber = Number(session.expiresAt);
  if (!Number.isFinite(expiresAtNumber) || expiresAtNumber < Math.floor(Date.now() / 1000)) return null;
  return {
    userId: String(session.userId),
    inviteHash: String(session.inviteHash || ""),
    plan: String(session.plan || "free"),
    label: String(session.label || "内测用户")
  };
}
