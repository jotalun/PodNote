...
- Transcript 下载新增 `.md` 选项，导出时会包含 YAML frontmatter、节目信息和 transcript 正文。
- 保留原来的 `.txt` 下载，用户可以按保存场景选择格式。

## 0.19.1 - 2026-05-19

### 修复

- 移除浏览器端旧的每日 300 分钟硬限制，避免 owner 套餐被前端本地用量误拦截。
- 转写额度统一交给服务端 KV、用户套餐和全站每日预算判断。

## 0.19.0 - 2026-05-18

### 新增

- `owner` 套餐默认额度放宽：每月转写 3000 分钟、每日转写 600 分钟、单集 240 分钟、每月分析 1000 次。
- 新增 `npm run invites -- 数量 套餐 标签`，可批量生成适合放进 `PODNOTE_INVITE_CODES` 的邀请码。

### 文档

- 补充陌生人内测的邀请码策略：一人一个独立 `free` 邀请码，你自己使用 `owner`。

## 0.18.0 - 2026-05-18

### 新增

- 顶部新增 `额度用量`，内测用户可以查看本月转写、今日转写、DeepSeek 分析次数、全站今日转写和全站今日预算。
- 额度面板复用 `/api/me`，不新增敏感接口，也不会显示任何 API Key。
- 转写或分析成功后，顶部额度会继续刷新为最新剩余额度。

### 改进

- 文档同步说明额度面板，减少内测用户不理解额度限制的问题。

## 0.17.0 - 2026-05-18

### 新增

- 顶部新增 `内测说明`，内测用户可以直接查看完整使用流程和注意事项。
- 顶部新增 `配置检查`，用于检查邀请码、登录密钥、Cloudflare KV、DeepSeek、OpenAI、Deepgram 和全站预算配置状态。
- 新增 `/api/config`，本地和 Cloudflare 线上版都可以返回不含密钥原文的配置快照。

### 改进

- 文档同步补充配置检查页、内测说明页和当前部署排查方式。

## 0.16.2 - 2026-05-18

### 新增

- 服务端 transcript 缓存继续保留：同一集命中缓存时不会重复调用 OpenAI / Deepgram。
- 新增 `PODNOTE_GLOBAL_DAILY_COST_USD` 环境变量，用于设置全站每日美元预算硬上限。
- 全站每日预算会按转写预估成本检查，超过后直接拒绝新的转写请求。

## 0.16.1 - 2026-05-18

### 修复

- 登录页不再显示环境变量名，改成普通内测提示。
- 如果还没有配置正式邀请码，临时允许使用 `123456` 作为默认内测码进入；一旦配置正式邀请码，这个临时码就不会生效。
...
- 去掉 transcript 区域里多余的空状态开头，改成更直接的操作指引。

## 0.15.2 - 2026-05-18

### 修复

- 初始示例节目不再把 sample transcript 填入 `获取 transcript` 输入区。
- 未导入真实节目时，transcript 输入区不再预填内容，并显示下一步操作指引。
- 播放器字幕区在初始状态只显示导入提示，避免把示例字幕误认为真实分析素材。

## 0.15.1 - 2026-05-18

### 改进

- Markdown 下载区改为空状态，不再在导入节目或加载 transcript 时自动塞入本地模板。
- 只有点击 `DeepSeek 分析` 并成功返回后，下载区才显示 Obsidian Markdown。
- 复制和下载按钮在 Markdown 生成前会保持不可用，避免误下载草稿。
- 移除主界面的 `本地整理` 按钮，防止本地模板和 AI 分析结果混在一起。

## 0.15.0 - 2026-05-18

### 改进

- 将完整字幕时间线移入播放器卡片，不再放在左侧独立面板里。
- 字幕同步滚动现在只滚动播放器内的字幕容器，不再触发整个网页跟着跳动。
- Deepgram 如果只返回很长的 sentence，会按句子时长做近似切分，避免整段 transcript 挤在一个时间戳下面。
- 更新 transcript 缓存版本，避免继续读取旧的长段落缓存。
