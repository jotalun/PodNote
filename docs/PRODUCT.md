...
PodNote 是一个面向播客重度用户的知识整理工具。它把播客 RSS 导入、播放、transcript 和 AI Markdown 笔记放到一个简单流程里。
...
首页顶部就是 RSS 输入框，用户第一眼就能知道从哪里粘贴链接。
...
产品默认把 transcript 当作原始材料。右侧有一个明确的 `Transcript 原文` 输入区，可以手动粘贴节目转写文本。
...
部署版配置好 Cloudflare 环境变量 `DEEPSEEK_API_KEY` 后，点击右侧的 `DeepSeek 分析`，系统会把当前 transcript 发给后端，再由后端调用 DeepSeek。
...
在页面顶部 `粘贴播客 RSS 链接` 输入框里填入 RSS 地址，然后点击 `导入播客`。
