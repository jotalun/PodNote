...
网站现在使用邀请码登录。未登录时，页面和 API 都不能直接访问。

每个邀请码会绑定一份用户额度。默认额度可以通过环境变量调整：
...
陌生人内测建议使用 `free`，你自己使用 `owner`。无论哪个套餐，仍然会受到全站每日美元预算保护。

页面顶部会显示当前邀请码的剩余转写分钟数和 DeepSeek 分析次数。点击 `额度用量` 可以查看更完整的用量面板，包括本月转写、今日转写、本月 DeepSeek 分析、全站今日转写和全站今日预算。

如果还没有配置正式邀请码，系统会临时允许使用 `123456` 作为内测默认邀请码，方便先进入页面；配置正式邀请码后，默认内测码会自动失效。

### 0.1 内测说明、额度用量与配置检查

顶部有三个辅助入口：

- `内测说明`：给内测用户看的使用说明，解释从粘贴链接、获取 transcript、生成笔记到下载 Markdown 的完整流程。
- `额度用量`：给内测用户看的用量面板，显示当前套餐、本月/今日已用量、剩余额度和全站预算保护状态。
- `配置检查`：给维护者看的部署检查页，会检查邀请码、登录密钥、Cloudflare KV、DeepSeek、OpenAI、Deepgram 和全站预算是否配置好。

配置检查只显示是否配置完成和额度上限，不会显示任何 API Key 或密钥原文。
...
初始示例节目不会预填 sample transcript。未导入真实节目时，这个区域只显示操作指引，避免把示例内容误认为真实文字稿。
...
如果找到，会自动填入 transcript 输入区，并刷新播放器内的字幕时间线。Markdown 下载区会继续保持空状态，直到点击 `DeepSeek 分析`。
...
- 额度记录当前使用 Cloudflare KV，适合内测和轻量公开使用；如果后续有正式会员、订单和账单流水，可以升级到 D1。
- 付费或私密音频不会被转写。
...
- 查看逐句中文 transcript。
- 播放真实音频时，播放器内字幕时间线会按当前播放时间自动高亮并滚动。
- 播放器卡片内会同步显示当前字幕，方便像听歌看歌词一样跟读。
- 点击某句高亮。
- 切换中英对照展示。

导入真实 RSS 或小宇宙公开音频后，点击 transcript 时间戳会跳转到音频对应位置。播放时，当前时间对应的字幕会自动同步到播放器卡片，并且只滚动播放器内部的字幕列表，不会带着整个页面跳动。
...
右侧默认是空状态。只有 `DeepSeek 分析` 成功返回后，才会出现可复制、可下载的 Markdown。
...
- DeepSeek 分析。
- 邀请码登录和额度保护。
- Cloudflare KV 持久化用户额度和 transcript 缓存。
- 内测说明页和配置检查页。
- Markdown 下载。
...
如果是第一次部署，可以先点击页面顶部的 `配置检查`，确认 DeepSeek、OpenAI、Deepgram、KV 和预算都已经配置好。
...
点击 `DeepSeek 分析`，让 DeepSeek 基于完整 transcript 生成 Obsidian Markdown。分析成功前，右侧下载区不会显示草稿内容。
...
- OPML 暂时不在主界面显示。
- 配置检查只能判断环境变量或绑定是否存在，不会验证 API Key 是否真的有余额。
